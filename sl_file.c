...
/*   Updated: 2023/06/27 01:17:26 by tponutha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**sb_get_box(char *path, t_list **head)
{
	int		i;
	int		fd;
	int		len;
...
void sl_mapping(char *path, t_data *data)
{
	...
	data->head = NULL;
	data->map = sb_get_box(path, &data->head);
	print_map(data->map);
	if (data->map == NULL)
